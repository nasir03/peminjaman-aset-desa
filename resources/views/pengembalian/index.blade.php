                ...
                <table class="modern-table table table-bordered">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ ucfirst($pengembalian->kondisi_asset ?? '-') }}</td>
                            <td>{{ $pengembalian->catatan_pengembalian ?? '-' }}</td>
                            <td>
                                @if($pengembalian->foto_kondisi)
                                    <a href="{{ asset('storage/' . $pengembalian->foto_kondisi) }}" target="_blank">
                                        <img src="{{ asset('storage/' . $pengembalian->foto_kondisi) }}" alt="Foto Kondisi" width="60" height="60" style="object-fit: cover; border-radius: 4px;">
                                    </a>
                                @else
   ...
   title="Bayar Denda">
    <i class="fas fa-money-bill-wave"></i> Bayar
            ...
            <form action="{{ route('pengembalian.destroy', $pengembalian->id_pengembalian) }}" method="POST" class="d-inline" onsubmit="return confirm('Yakin ingin menghapus data ini?')">
                ...
                <a href="{{ route('pengembalian.edit', $pengembalian->id_pengembalian) }}" class="btn-icon btn-warning" title="Edit">
